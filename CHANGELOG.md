...
- A failed request reaches the model as a result rather than as a transport error. The same sibling relationship meant a rate limit or an unreachable server escaped a live tool entirely, and the caller saw a protocol-level failure with no explanation.
