...
# or suspiciously large files that might contain real bike/personal data.
#
...
#   *.db.<anything>      (e.g. bosch_flow.db.bak)
...
if [ "$errors" -ne 0 ]; then
    echo ""
    echo "Commit rejected. Bike data and credentials must never be committed."
    exit 1
fi
