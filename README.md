...
Fetches all data types from the EU Data Act API and stores them locally. You can also
use the `bosch_sync` MCP tool, or rely on automatic sync (each `get_*` tool triggers
a sync if data is stale).
