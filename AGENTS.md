...
- **`require_auth` is the only handler a tool has.** The four API exception types are siblings off `Exception`, so a live read catches none of them: without the gate's own `try`, a rate limit or an unreachable server reached the MCP client as a transport error rather than a tool result. `run_sync` needs its own handlers for the same reason, including a trailing catch-all - `except BoschAPIError` does not catch a 429. Pinned by `TestTheAuthGate` in `tests/test_helpers.py`, which also pins the refusal when there are no credentials; nothing covered either half before
