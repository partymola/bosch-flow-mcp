...
- **API**: `api.py` - GET wrapper with thread-safe token refresh (5-min expiry buffer) and typed exceptions: `BoschAuthError`, `BoschRateLimitError`, `BoschAPIError`, `BoschForbiddenError`. **They are siblings off `Exception`, not a hierarchy** - `except BoschAPIError` does not catch a 429, which is why every layer that handles failures names `BoschRateLimitError` separately
